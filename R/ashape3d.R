...
function (x, alpha, pert = FALSE, eps = 1e-09) 
{
    ...
    if (inherits(x, "ashape3d")) {
        ...
        fm123 <- .Fortran("Fm123", x, as.integer(n), t1u, t2u, 
        ...
        if (any(m1230 == 0) & pert) {
            ...
            flag <- 1
            while (any(m1230 == 0)) {
                x <- x + rnorm(length(x), sd = sd(x) * eps)
                tc <- matrix(delaunayn(x), nc = 4)
                ntc <- dim(tc)[1]
                ORDM <- matrix(as.integer(0), ntc, 4)
                storage.mode(tc) <- "integer"
                storage.mode(ORDM) <- "integer"
                tc <- .Fortran("SORTM", ORDM, as.integer(ntc), 
                  as.integer(4), tc, PACKAGE = "alphashape3d")[[1]]
                tc.aux <- matrix(1:(4 * ntc), ncol = 4)
                tri <- rbind(tc[, -4], tc[, -3], tc[, -2], tc[, 
                  -1])
                t1 <- tri[, 1]
                t2 <- tri[, 2]
                t3 <- tri[, 3]
                ...
                m123 <- numeric(ntri)
                storage.mode(m123) <- "double"
                fm123 <- .Fortran("Fm123", x, as.integer(n), 
                  t1u, t2u, t3u, as.integer(ntri), m123, PACKAGE = "alphashape3d")
                m123 <- fm123[[7]]
                m1230 <- +m123[ind.tri[tc.aux[, 1]]] - m123[ind.tri[tc.aux[, 
                  2]]] + m123[ind.tri[tc.aux[, 3]]] - m123[ind.tri[tc.aux[, 
                  4]]]
        ...
        fmk0 <- .Fortran("Fmk0", x, as.integer(n), e1u, e2u, 
        ...
        fmij0 <- .Fortran("Fmij0", x, as.integer(n), t1u, t2u, 
        ...
        fmijk0 <- .Fortran("Fmijk0", x4, as.integer(n), tc, as.integer(ntc), 
            tc.aux, ind.tri, as.integer(ntri), m230, m130, m120, 
            m2340, m1340, m1240, PACKAGE = "alphashape3d")
        m2340 <- fmijk0[[11]]
        m1340 <- fmijk0[[12]]
        m1240 <- fmijk0[[13]]
        m1234 <- -(-x4[tc[, 4]] * m123[ind.tri[tc.aux[, 1]]] + 
            x4[tc[, 3]] * m123[ind.tri[tc.aux[, 2]]] - x4[tc[, 
            2]] * m123[ind.tri[tc.aux[, 3]]] + x4[tc[, 1]] * 
            m123[ind.tri[tc.aux[, 4]]])
        rho.sq <- (0.25 * (m2340^2 + m1340^2 + m1240^2 + 4 * 
            m1230 * m1234)/m1230^2)
        if (any(rho.sq < 0)) {
            ind <- which(rho.sq < 0)
            v1 <- x[tc[, 1], ]
            ...
            for (i in 1:length(ind)) {
                ...
                Dx[i] <- det(cbind(nor, tetra[, 2:3], rep(1, 
                  4)))
                Dy[i] <- -det(cbind(nor, tetra[, c(1, 3)], rep(1, 
                  4)))
            ...
            rho.sq[ind] <- 0.25 * (Dx^2 + Dy^2 + Dz^2 - 4 * a * 
                ct)/a^2
        }
        rho4 <- sqrt(rho.sq)
        mu4 <- rho4
        ...
        mus3 <- .Fortran("int3", as.integer(ntri2), as.double(rf1), 
        ...
        pu1 <- sign[i1] * m2340[in.tc[i1]] * m230[1:ntris] + 
            sign[i1] * m1340[in.tc[i1]] * m130[1:ntris] + sign[i1] * 
            m1240[in.tc[i1]] * m120[1:ntris] - 2 * sign[i1] * 
            m1230[in.tc[i1]] * m123[1:ntris]
        pu2 <- sign[i2] * m2340[in.tc[i2]] * m230[1:ntris] + 
            sign[i2] * m1340[in.tc[i2]] * m130[1:ntris] + sign[i2] * 
            m1240[in.tc[i2]] * m120[1:ntris] - 2 * sign[i2] * 
            m1230[in.tc[i2]] * m123[1:ntris]
        at3 <- (pu1 > 0 | pu2 > 0)
        pu3 <- sign[ih] * m2340[in.tc[ih]] * m230[(ntris + 1):ntri] + 
            sign[ih] * m1340[in.tc[ih]] * m130[(ntris + 1):ntri] + 
            sign[ih] * m1240[in.tc[ih]] * m120[(ntris + 1):ntri] - 
            2 * sign[ih] * m1230[in.tc[ih]] * m123[(ntris + 1):ntri]
        at3 <- c(at3, pu3 > 0)
        ...
        mus2 <- .Fortran("int2", dup, as.integer(ned), as.integer(ntri), 
            in.trio, auxmu3, Mu3, mu2, mu2up, tri.aux, aux1, 
            ind.ed, num.rho2, mk0, at, PACKAGE = "alphashape3d")
        mu2 <- mus2[[7]]
        Mu2 <- mus2[[8]]
        at2 <- mus2[[14]]
        ...
        mus1 <- .Fortran("int1", dup1, as.integer(nvt), as.integer(ned), 
