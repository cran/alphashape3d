...
function (x, alpha, pert = FALSE, eps = 1e-09) 
{
    ...
    if (inherits(x, "ashape3d")) {
        ...
        while (flag == 1) {
            ...
            fm123 <- .Fortran("Fm123", x, as.integer(n), t1u, 
                t2u, t3u, as.integer(ntri), m123, PACKAGE = "alphashape3d")
            m123 <- fm123[[7]]
            ...
            if (flag == 0) {
                ...
                fmk0 <- .Fortran("Fmk0", x, as.integer(n), e1u, 
                  e2u, as.integer(ned), mk0, num2, PACKAGE = "alphashape3d")
                mk0 <- fmk0[[6]]
                num.rho2 <- fmk0[[7]]
                ...
                fmij0 <- .Fortran("Fmij0", x, as.integer(n), 
                  t1u, t2u, t3u, as.integer(ntri), tri.aux, ind.ed, 
                  as.integer(ned), mk0, m23, m13, m12, num.rho2, 
                  num3, PACKAGE = "alphashape3d")
                m230 <- fmij0[[11]]
                m130 <- fmij0[[12]]
                m120 <- fmij0[[13]]
                num.rho3 <- fmij0[[15]]
            ...
            if (flag == 0) {
                ...
                fmijk0 <- .Fortran("Fmijk0", x4, as.integer(n), 
                  tc, as.integer(ntc), tc.aux, ind.tri, as.integer(ntri), 
                ...
                mus2 <- .Fortran("int2", dup, as.integer(ned), 
                  as.integer(ntri), in.trio, auxmu3, Mu3, mu2, 
                  mu2up, tri.aux, aux1, ind.ed, num.rho2, mk0, 
                  at, PACKAGE = "alphashape3d")
                mu2 <- mus2[[7]]
                Mu2 <- mus2[[8]]
                at2 <- mus2[[14]]
                ...
                mus1 <- .Fortran("int1", dup1, as.integer(nvt), 
                  as.integer(ned), in.edo, auxmu2, Mu2, mu1, 
                  mu1up, PACKAGE = "alphashape3d")
                mu1 <- mus1[[7]]
                Mu1 <- mus1[[8]]
